...
VARS2="environments/$(ENV)/$(ENV)-$(TIER).tfvars"
REGION=us-east-1
CURRENT_FOLDER=$(shell basename "$$(pwd)")
S3_BUCKET="$(ENV)-dr-$(REGION)-terraform-state"
DYNAMODB_TABLE="$(ENV)-$(TIER)-dr-$(REGION)-terraform"
WORKSPACE="$(ENV)-$(TIER)-$(REGION)"
BOLD=$(shell tput bold)
RED=$(shell tput setaf 1)
GREEN=$(shell tput setaf 2)
YELLOW=$(shell tput setaf 3)
RESET=$(shell tput sgr0)
BUCKET_KEY = $(shell basename "$$(pwd)")
KMS_KEY_ID="arn:aws:kms:us-east-1:505028564994:key/53139ce7-a4ef-413a-8b90-ae7163ddf59b"

help:
	@grep -E '^[a-zA-Z_-]+:.*?## .*$$' $(MAKEFILE_LIST) | sort | awk 'BEGIN {FS = ":.*?## "}; {printf "\033[36m%-30s\033[0m %s\n", $$1, $$2}'

validate: ## validates that terraform files work
	@terraform validate -var-file=$(VARS) -var-file=$(VARS2)
...
set-env:
	...
	@if [ ! -f "$(VARS)" ]; then \
	    ...
	    echo -e "\nPulling fresh $(ENV)-$(TIER).tfvars from s3://$(ENV)-dr-$(REGION)-terraform-state/$(BUCKET_KEY)/\n"; \
	    echo aws s3 cp s3://$(ENV)-dr-$(REGION)-terraform-state/$(CURRENT_FOLDER)/$(ENV)-$(TIER).tfvars $(VARS); \
	    aws s3 cp s3://$(ENV)-dr-$(REGION)-terraform-state/$(CURRENT_FOLDER)/$(ENV)-$(TIER).tfvars $(VARS); \
		exit 1; \
	 fi
...
prep: set-env ## Prepare a new workspace (environment) if needed, configure the tfstate backend, update any modules, and switch to the workspace
	...
	@aws ec2 --profile=$(AWS_PROFILE) describe-key-pairs | jq -r '.KeyPairs[].KeyName' | grep "$(ENV)_$(TIER)_infra_key" > /dev/null 2>&1; \
	if [ $$? -ne 0 ]; then \
	    echo "$(BOLD)$(RED)EC2 Key Pair $(ENV)_$(TIER)_infra_key was not found$(RESET)"; \
	    read -p '$(BOLD)Do you want to generate a new keypair? [y/Y]: $(RESET)' ANSWER && \
    	if [ "$${ANSWER}" == "y" ] || [ "$${ANSWER}" == "Y" ]; then \
	        mkdir -p ~/.ssh; \
	        ssh-keygen -t rsa -b 4096 -N '' -f ~/.ssh/$(ENV)_$(TIER)_infra_key; \
        	aws ec2 --profile=$(AWS_PROFILE) import-key-pair --key-name "$(ENV)_$(TIER)_infra_key" --public-key-material "file://~/.ssh/$(ENV)_$(TIER)_infra_key.pub"; \
    	fi; \
	  else \
	      echo "$(BOLD)$(GREEN)EC2 Key Pair $(ENV)_$(TIER)_infra_key exists$(RESET)";\
	  fi
